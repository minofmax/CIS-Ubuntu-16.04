...
@test "5.3.23 Ensure SSH MaxSessions is limited (Automated)" {
    run bash -c "sshd -T -C user=root -C host=\"$(hostname)\" -C addr=\"$(grep $(hostname) /etc/hosts | awk '{print $1}')\" | grep -i maxsessions"
    echo {"\"output\"": "\"$output\""}
    [ "$status" -eq 0 ]
    [[ "$output" == "maxsessions 10" ]]
    run bash -c "grep -Ei '^\s*MaxSessions\s+(1[1-9]|[2-9][0-9]|[1-9][0-9][0-9]+)' /etc/ssh/sshd_config"
    echo {"\"output\"": "\"$output\""}
    [ "$status" -ne 0 ]
    [[ "$output" == "" ]]
}
